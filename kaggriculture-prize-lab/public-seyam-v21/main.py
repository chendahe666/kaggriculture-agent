...
_V21_LATE_DECISION_STEP = 577
_V21_CAPITAL_GAP_THRESHOLD = -5_000
_V21_LATE_ABSTAIN_BY_SEAT = {}
...
def _v21_late_abstain(obs, step):
    """Latch late abstention when the public rival is already far behind."""
    seat = _v18s_number(_get(obs, "player", 0))
    if step == 0:
        _V21_LATE_ABSTAIN_BY_SEAT.pop(seat, None)
    if step < _V21_LATE_DECISION_STEP:
        return False
    if seat not in _V21_LATE_ABSTAIN_BY_SEAT:
        farms = list(_get(obs, "farms", []) or [])
        if len(farms) != 2 or seat not in (0, 1):
            _V21_LATE_ABSTAIN_BY_SEAT[seat] = False
        else:
            own_money = float(_get(farms[seat], "money", 0) or 0)
            rival_money = float(_get(farms[1 - seat], "money", 0) or 0)
            _V21_LATE_ABSTAIN_BY_SEAT[seat] = (
                rival_money - own_money <= _V21_CAPITAL_GAP_THRESHOLD
            )
    return _V21_LATE_ABSTAIN_BY_SEAT[seat]


def _v18s_recovery_sweep(obs, action):
    copied = _copy_action(action)
    step = _v18s_number(_get(obs, "step", 0))
    if _v21_late_abstain(obs, step):
        return copied

...
# ===========================================================================
# K320 adaptive rank-1 controller
#
# Keep V20's Kawashigi programmes intact.  Only add:
#   1. route lock at shop-unlock checkpoints (no mid-tape splicing)
#   2. farm rebind after the herd exists, and only onto the programme we
#      have already built
#   3. four-turn clone preemption with a persistent repay ledger
#   4. a late, loss-only liquidation nudge
# ===========================================================================
_K320_SHOP_ROUTE = _kawa_route_label
_K320_BASE_AGENT = agent

_K320_ROUTE_LOCK = {0: None, 1: None}
_K320_LAST = {0: -1, 1: -1}
_K320_TARGETS = {
    "10c4s_3q": (10, 4, 3),
    "8c6s_3q": (8, 6, 3),
    "6c8s_3q": (6, 8, 3),
    "6c12s_4q_first_yarn": (6, 12, 4),
    "6c12s_4q_second_yarn": (6, 12, 4),
}


def _k320_reset(seat, step):
    if step == 0 or step < _K320_LAST[seat]:
        _K320_ROUTE_LOCK[seat] = None
    _K320_LAST[seat] = step


def _k320_counts(farm):
    cows = sheep = 0
    for row in list(_get(farm, "tiles", []) or []):
        for tile in list(row or []):
            ...
            if tile.get("animal") == "COW":
                ...
                sheep += 1
    lands = len(_get(farm, "unlocked_quadrants", []) or [])
    return cows, sheep, lands


def _kawa_route_label(obs):
    step = int(_get(obs, "step", 0) or 0)
    seat = _seat(obs)
    _k320_reset(seat, step)
    shop_label = _K320_SHOP_ROUTE(obs)
    shops = list(((_get(obs, "town", {}) or {}).get("unlocked_shops", []) or []))
    locked = _K320_ROUTE_LOCK.get(seat)

    # After the herd is down, stay on the programme the live farm matches.
    # Yarn programmes are irreversible: once the shop order names one,
    # stay on that tape.  Non-yarn labels keep following live shop logic
    # until the third shop exists so a late Yarn Store can still select
    # the 6c8s branch.
    if locked and str(locked).startswith("6c12s"):
        return locked
    if str(shop_label).startswith("6c12s"):
        _K320_ROUTE_LOCK[seat] = shop_label
        return shop_label
    if locked:
        return locked
    if len(shops) >= 3 or step >= 216:
        _K320_ROUTE_LOCK[seat] = shop_label
    return shop_label
...
def _future_sells(obs, step):
    ...
    legacy = _kawa_use_legacy_layout(obs)
    if legacy:
        horizon = 4
    elif clone <= 6:
        horizon = 2
...
def _repay_shift(obs, action, step):
    ...
    for raw in action.get("market", []) or []:
        ...
        market.append(order)
    action["market"] = market
    state["due"] = {item: qty for item, qty in due.items() if qty > 0}
    state["due_step"] = step + 1 if state["due"] else -1
    return action


_K320_ONE_SHOT = {"WHEAT": 2, "CARROT": 2}
_K320_LAST_DAY = 29


def _k320_plant_count(action, crop):
    total = 0
    orders = [action.get("farmer", ["PASS"]), *list(action.get("hands") or [])]
    for order in orders:
        if isinstance(order, (list, tuple)) and len(order) >= 2 and order[0] == "PLANT" and order[1] == crop:
            total += 1
    return total


def _k320_trim_unfinishable_seeds(obs, action, step):
    """Do not buy one-shot seeds that cannot be planted and harvested."""
    private = _get(obs, "private", {}) or {}
    seeds = {str(k): max(0, int(v or 0)) for k, v in dict(_get(private, "seeds", {}) or {}).items()}
    tape = _kawa_actions(obs)
    action = _copy_action(action)
    market = []
    for raw in action.get("market") or []:
        order = list(raw)
        if len(order) >= 3 and order[0] == "BUY_SEED" and order[1] in _K320_ONE_SHOT:
            crop = order[1]
            last_plant_day = _K320_LAST_DAY - _K320_ONE_SHOT[crop]
            later = 0
            for ahead in range(step + 1, len(tape)):
                if ahead // 24 > last_plant_day:
                    break
                later += _k320_plant_count(tape[ahead], crop)
            need = _k320_plant_count(action, crop) + later
            have = seeds.get(crop, 0)
            allowed = max(0, need - have)
            quantity = min(max(0, int(order[2] or 0)), allowed)
            if quantity <= 0:
                continue
            order[2] = quantity
...
def kaggriculture_k320_adaptive_rank1(obs, configuration=None):
    action = _K320_BASE_AGENT(obs)
    step = min(max(0, int(_get(obs, "step", 0) or 0)), 719)
    return _k320_trim_unfinishable_seeds(obs, action, step)


agent = kaggriculture_k320_adaptive_rank1
__version__ = "K320-adaptive-rank1-no-late-seed"
